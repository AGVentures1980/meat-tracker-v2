    ...
    return (
        <div className="p-6">
            ...
            <div className="hidden print:block mb-8 text-black">
                <h1 className="text-4xl font-bold uppercase mb-2">Meat Prep Guide</h1>
                <div className="flex justify-between text-sm border-b-2 border-black pb-2">
                    <span>Date: {new Date().toLocaleDateString()}</span>
                    <span>Projected Guests: {forecast}</span>
            ...
            {!loading && !error && prepData && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 print:grid-cols-2 print:gap-4 print:text-black">
                    {prepData.prep_list.map((item: any) => (
                        <div key={item.protein} className="bg-[#121212] border border-[#333] rounded p-4 hover:border-[#00FF94]/50 transition-colors group print:bg-white print:border-black print:break-inside-avoid">
                            <div className="flex justify-between items-start mb-3">
                                <h3 className="font-bold text-lg text-white group-hover:text-[#00FF94] transition-colors print:text-black">{item.protein}</h3>
                                <span className="text-xs bg-[#222] text-gray-400 px-2 py-1 rounded font-mono print:border print:border-black print:bg-transparent print:text-black">{item.mix_percentage}</span>
                            </div>

                            <div className="space-y-4">
                                <div className="flex justify-between items-end border-b border-[#333] pb-2 print:border-black">
                                    <span className="text-xs text-gray-500 uppercase print:text-black/60">Prep Qty</span>
                                    <div className="text-right">
                                        <div className="text-3xl font-black text-white leading-none print:text-black">{Math.ceil(item.recommended_units)}</div>
                                        <div className="text-[10px] text-gray-500 uppercase mt-1 print:text-black/60">Pieces / Packs</div>
                                    </div>
                                </div>

                                <div className="flex justify-between items-center text-xs">
                                    <span className="text-gray-500 print:text-black/60">Total Weight</span>
                                    <span className="text-gray-300 font-mono print:text-black">{item.recommended_lbs} lbs</span>
                                </div>
                                <div className="flex justify-between items-center text-xs">
                                    <span className="text-gray-500 print:text-black/60">Avg Unit</span>
                                    <span className="text-gray-300 font-mono print:text-black">{item.avg_weight} lbs</span>
                                </div>
                            </div>
                        </div>
                    ))}
