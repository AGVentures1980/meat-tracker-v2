...
export let intakeQueue: Queue;
export let intakeWorker: Worker | null = null;

if (hasRedis && redisConnection) {
    intakeQueue = new Queue('data-intake', { connection: redisConnection });

    intakeWorker = new Worker('data-intake', async (job: Job) => {
        ...
        await prisma.$transaction(async (tx) => {
            await tx.intakeJob.upsert({
                ...
                create: { dataset_item_id: itemId, attempts: 1, error_message: null, worker_id: intakeWorker?.id || 'unknown' }
            });
    ...
    }, { connection: redisConnection, concurrency: parseInt(process.env.WORKER_CONCURRENCY || '10') });
    ...
    console.log('[Intake Queue] Running in Mock mode (Local Development)');
    intakeQueue = {
        add: async (jobName: string, data: any, opts?: any) => {
            console.log(`[Queue Mock] Added intake job:`, jobName, data);
            ...
            if (itemId) {
                setTimeout(async () => {
                    try {
                        const item = await prisma.goldenDatasetItem.findUnique({ where: { id: itemId } });
                        if (item && !['VALIDATED', 'QUARANTINED', 'REJECTED', 'COMPLETED'].includes(item.status)) {
                            await prisma.goldenDatasetItem.update({
                                where: { id: itemId },
                                data: {
                                    status: 'VALIDATED',
                                    parsed_output: { msg: 'parsed (mocked)' },
                                    normalized_output: { msg: 'normalized (mocked)' },
                                    validation_output: { msg: 'success (mocked)' },
                                    processed_at: new Date()
                                }
                            });
                            console.log(`[Queue Mock] Processed dataset item ${itemId} successfully (mocked).`);
                        }
                    } catch (e: any) {
                        console.error(`[Queue Mock] Failed processing item ${itemId}:`, e.message);
                    }
                }, 1000);
            }
