...
export let ocrWorker: Worker | null = null;

if (hasRedis && redisConnection) {
    ocrWorker = new Worker('ocr-processor', async job => {
    ...
    console.log('[OCR WORKER] Skipping Worker initialization because REDIS_URL is not provided (Mock Mode).');
}
