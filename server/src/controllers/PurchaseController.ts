                ...
                try {
                    ...
                    if (isImage) {
                        ...
                        if (userCompanyId === 'fdc-main') {
                            ...
                            providerSpecificRules = `CRITICAL SYSCO INVOICE RULES:\n1. QUANTITY (LBS): Use T/WT= if present. If NO T/WT= exists (Fixed Weight), calculate total lbs by multiplying Case Qty by Pack Size. *CRITICAL OCR TYPO*: The OCR often misreads '2 15 LB' Bacon as '215 LB' or '215'. If you see '215 LB' for Bacon, it ALWAYS means 2 packs of 15 lbs = 30 lbs total per case. NEVER output 215 as quantity for Bacon.\n2. PRICE: Extract unit price per lb, NOT the extended line total. If you calculate quantity as 30 lbs and the line total is 93.38, price_per_lb is 93.38 / 30 = 3.11.\n3. Common Abbreviations: 'Picanha' is 'Coulotte' or 'COULOT'. 'Flank' is 'Fraldinha'. 'Beef Rib' is 'SHORT RIB'.`;
                        }
