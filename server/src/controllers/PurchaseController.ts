                ...
                try {
                    ...
                    if (isImage) {
                        ...
                        if (userCompanyId === 'fdc-main') {
                            ...
                            providerSpecificRules = `CRITICAL SYSCO INVOICE RULES:\n1. QUANTITY (LBS): *CRITICAL* The true Catch Weight ('T/WT=') is often printed on the line IMMEDIATELY BELOW the product description. You MUST search the line directly below the item to find its corresponding 'T/WT=' value. DO NOT accidentally take the 'T/WT=' from the item above it. ALWAYS use the exact value immediately following 'T/WT=' for quantity. *CRITICAL WARNING*: The line below sometimes starts with numbers in Kg (e.g. '34.830 40.680 T/WT= 75.510'). You MUST IGNORE those first numbers and ONLY use the 'T/WT=' value (75.51). NEVER extract the Kg value. You MUST IGNORE any inline numbers like '160#AVG' or '52 PC'. If NO T/WT= exists (Fixed Weight), look at the item description for pack sizes like '1/25' (which means 25 lbs per case) or '4/10' (4x10=40 lbs per case). Calculate total lbs by multiplying Case Qty by that lb value (e.g., 4 CS of '1/25' = 100 lbs). Alternatively, look for an explicit 'XXX LB' on the line (e.g., '100 LB') and use that. *OZ CALCULATION*: If the description contains 'OZ' (e.g., '96 3.6 OZ' meaning 96 pieces of 3.6oz each), calculate total weight in lbs: (Pieces * Ounces / 16) * Case Qty. Example: 4 CS of '96 3.6 OZ' = 4 * (96 * 3.6 / 16) = 86.4 LBS. *CRITICAL TYPO*: OCR often misreads '96 3.6 OZ' as '963.6 OZ'. Treat '963.6 OZ' as '96 3.6 OZ' and calculate the weight as 21.6 lbs per case. *CRITICAL OCR TYPO*: The OCR often misreads '2 15 LB' Bacon as '215 LB' or '215'. If you see '215 LB' for Bacon, it ALWAYS means 2 packs of 15 lbs = 30 lbs total per case. *CRITICAL OCR TYPO*: OCR often misreads '4 10 LB' as '410 LB'. If you see '410 LB', it ALWAYS means 4 packs of 10 lbs = 40 lbs per case and YOU MUST calculate 40 lbs * Case Qty. NEVER output 410 as quantity.\n2. PRICE: Extract unit price per lb, NOT the extended line total and NOT the case price. If you calculate quantity as 100 lbs and the line total is 380.00, price_per_lb MUST BE 380.00 / 100 = 3.80. NEVER output the per-case price (e.g. 95.00) as price_per_lb.\n3. CLEAN RAW TEXT: To prevent user confusion, you MUST scrub/delete confusing piece counts (like '52 PC') or estimated averages (like '160#AVG') from the 'raw_text' output, completely replacing them with the exact true Catch Weight (e.g. '56 LBS', '177.1 LBS').\n4. Common Abbreviations: 'Picanha' is 'Coulotte' or 'COULOT'. 'Flank' is 'Fraldinha'. 'Beef Rib' is 'SHORT RIB'. 'CHICKEN BRST' is 'Chicken Breast'. 'Chicken CVP' or 'Chicken Drumstick' or 'Chicken' should always be collected if it's meat.`;
                        }
